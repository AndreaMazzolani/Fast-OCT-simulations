...
% Folder to add to the path
saved_data = [path_str,'\My_papers_code\Fast_OCT_simulations\git_hub\saved_data'];

% folder_data =   'Large_OCT_Cscan_new_Nk_173_W0_9_Nscanx_3_Nscany_3_Nv_640_Zmin_0_Zmax_200_zf_290_alpha_0_0_NL_1_NA_1.mat';
folder_data =   'Large_OCT_Cscan_new_Nk_865_W0_9_Nscanx_200_Nscany_200_Nv_312500_Zmin_0_Zmax_1e+03_zf_290_alpha_0_0_NL_1_NA_1.mat';
...
if not(isfolder(remote)) & (exist(str_folder) == 2)  
    ...
    Nscanx = 2; ceil(2*Max_scan/dscan); % Number of A-scan for each B-scan
...
%     ind_sp = find(x.^2+(z-z_f).^2 <= (10e-6).^2); rho(ind_sp) = rhoMax*10;
    ...
    str = [saved_data,'/Large_OCT_Cscan_new_Nk_',num2str(Nk),'_W0_',num2str(W_0*1e6),'_Nscanx_',num2str(Nscanx),'_Nscany_',num2str(Nscany),'_Nv_',num2str(Nv),'_Zmin_',num2str(minZ*1e6),'_Zmax_',num2str(maxZ*1e6,3),'_zf_',num2str(z_f*1e6,3),'_alpha_',str_a,'.mat'];
