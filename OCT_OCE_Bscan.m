...
if not(isfolder(remote)) & (exist(str_folder) == 2)  
    ...
    LX = 70e-6;     % Maximum lateral position of the scatterers in x direction (meters)
    LY = 70e-6;     % Maximum lateral position of the scatterers in y direction (meters)
    minZ = 000e-6;   % minimum of the scatterers axial position (meters)
    maxZ = 150e-6;  % maximum of the scatterer axial  position (meters) (meters)
...
if not(isfolder(remote))
    ...
    BU_CC = BU_CC1*5+BU_CC2; BU_CCz = BU_CCz1*5+BU_CCz2;
    MA = max( (abs(BU_CC(:))));   MAz = max( (abs(BU_CCz(:)))); 
    IZ_min = min(find(zbar > 0e-6)); IZ_max = max(find(zbar < 1000e-6)); IZ = IZ_min:IZ_max; Nz2 = length(IZ);
    
    figure (1); clf; ca1 = 0; ca2 = 0.3;
    ...
    figure (11); clf; ca1 = 0; ca2 = 0.3;
    subplot(1,2,1); imagesc_set(log(abs(BU_CC(IZ,:,3)./MA)),zbar(IZ),x_scan_vec); axis equal;   colormap('gray'); %caxis([ca1,ca2]); 
    subplot(1,2,2); imagesc_set(log(abs(BU_CCz(IZ,:,3)./MAz)),zbar(IZ),x_scan_vec); axis equal;   colormap('gray'); %caxis([ca1,ca2]);
...
%     subplot(1,2,2); pcshow(ptCloudz); set(gcf,'color','w'); colorbar;  set(gca, 'FontSize',NF','Color','w','XColor', [0 0 0], 'YColor', [0 0 0], 'ZColor', [0 0 0]);
    ...
    figure(2); clf; NF = 11; sgtitle('|C-scan|','FontSize',NF*2);
    subplot(1,2,1); pcshow(v_oct,(B_oct)); set(gcf,'color','w'); set(gca, 'FontSize',NF','Color','w','XColor', [0 0 0], 'YColor', [0 0 0], 'ZColor', [0 0 0]); colormap('gray');  xlabel('x (\mu m)');  ylabel('y (\mu m)');  zlabel('z (\mu m)'); caxis([ca1,ca2]); title('$z_f = 290 \mu m$');
    subplot(1,2,2); pcshow(v_oct,(B_octz)); set(gcf,'color','w'); colorbar;  set(gca, 'FontSize',NF','Color','w','XColor', [0 0 0], 'YColor', [0 0 0], 'ZColor', [0 0 0]); colormap('gray');  xlabel('x (\mu m)');  ylabel('y (\mu m)');  zlabel('z (\mu m)'); caxis([ca1,ca2]); title('$z_f = 750 \mu m$');

    figure(12); clf; NF = 11; sgtitle('|C-scan|','FontSize',NF*2);
    subplot(1,2,1); pcshow(v_oct,(B_oct)); set(gcf,'color','w'); set(gca, 'FontSize',NF','Color','w','XColor', [0 0 0], 'YColor', [0 0 0], 'ZColor', [0 0 0]);   xlabel('x (\mu m)');  ylabel('y (\mu m)');  zlabel('z (\mu m)'); caxis([ca1,ca2]); title('$z_f = 290 \mu m$');
    subplot(1,2,2); pcshow(v_oct,(B_octz)); set(gcf,'color','w'); colorbar;  set(gca, 'FontSize',NF','Color','w','XColor', [0 0 0], 'YColor', [0 0 0], 'ZColor', [0 0 0]);  xlabel('x (\mu m)');  ylabel('y (\mu m)');  zlabel('z (\mu m)'); caxis([ca1,ca2]); title('$z_f = 750 \mu m$');
