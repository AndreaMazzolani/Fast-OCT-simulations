function Y = func_cell2(X_cell,varargin); % It is linked to "precision_set2" function, and it calculates the double estimation from int
if not(isempty(X_cell));
X = X_cell{1}; sX = size(X); LX  = length(sX);
    ...
    iarg = 1;
    while num_arg2 >= iarg;
        eval(['if strcmp(varargin{',num2str(iarg),'},"all"); varargin{',num2str(iarg),'} = 1:sX(',num2str(iarg),'); end']); 
        eval(['Ind_',num2str(iarg),' = varargin{',num2str(iarg),'};']); iarg = iarg+1;
    end
    
    iarg = 1;
    while num_arg2 >= iarg & iarg <= LDX;
        eval(['if strcmp(varargin{',num2str(iarg),'},"all"); varargin{',num2str(iarg),'} = 1:sDX(',num2str(iarg),'); end ']); 
 
        if not(eval(['isequal(Ind_D',num2str(iarg),',1)']));
        eval(['Ind_D',num2str(iarg),' = varargin{',num2str(iarg),'};']); 
        end;  iarg = iarg+1;
    end
...
else disp('Compressed file is empty!'); Y = [];
end
end
