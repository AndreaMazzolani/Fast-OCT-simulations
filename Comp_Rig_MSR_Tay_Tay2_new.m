...
folder_data = 'Test_dataset_new_Nkm2_50_Nkm_248_NkTmax_3_Nvm_1000_Nvm2_100_NmT_3_Nind_k_10_large.mat';
% folder_data = ' ';

str_file = [saved_data,'\',folder_data];



% Folders to add to the path

if not(isfolder(remote))&(exist(str_file) == 2)
    DL = load(str_file); if (isfield(DL,"DL")); DL = DL.DL; end; unpackStruct(DL);
else
...
%  Nkm2 = 50; Nkm = 248;  NkTmax = 3;  Nvm = 1000; Nvm2 = 100; NmT = 3;
   
    Nind_k = 10; ind_vec_k = ceil([1:Nind_k]*Nkm/Nind_k); 
    str_save = [saved_data,'/Test_dataset_new_Nkm2_',num2str(Nkm2),'_Nkm_',num2str(Nkm),'_NkTmax_',num2str(NkTmax),...
...
%     save(str_save,'DL'); disp(str_save)
    str_save2 = [saved_data,'/Test_dataset_new_Nkm2_',num2str(Nkm2),'_Nkm_',num2str(Nkm),'_NkTmax_',num2str(NkTmax),...
